...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sql="INSERT INTO `customer`( `customer_name`, `address`, `division_id`, `city_id`, `pincode`, `contact_no`, `mobile_no`, `email_id`, `password`, `customer_type`, `status`) VALUES ('$_POST[customername]','$_POST[address]','$_POST[division]','$_POST[city]','$_POST[pincode]','$_POST[cntctnum]','$_POST[mblnum]','$_POST[email_id]','$_POST[password]','$_POST[customertype]','$_POST[status]')";	
			if(!mysqli_query($con,$sql))
			{
...
<div class="form-row">
	...
	<div class="col-md-6 form-group">
	division <font color="#FF0000"> *</font>
	  <span id='loaddivision'><select class="form-control"></select></span>
	</div>
...
var emailExp = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/; //Variable to validate Email ID
	...
	else if(document.frmcstreg.division.value == "")
	{
		alert("Kindly select the division..");
		document.frmcstreg.division.focus();
		return false;
	}	
...
function loaddivision(id) {
    if (id == "") {
        document.getElementById("loaddivision").innerHTML = "";
        return;
    } else { 
        ...
        xmlhttp.onreadydivisionchange = function() {
            if (xmlhttp.readydivision == 4 && xmlhttp.status == 200) {
                document.getElementById("loaddivision").innerHTML = xmlhttp.responseText;
            }
        };
        xmlhttp.open("GET","ajaxdivision.php?id="+id,true);
        xmlhttp.send();
    }
}
